...
export function calculateCorrectedGrid(
    ...
    // 판형이 판을 넘는지 계수 계산
    const ratioW = measuredW / maxGridWidth;
    const ratioH = measuredH / maxGridHeight;
    ...
    // 최종 범위 클리핑
    correctedW = Math.max(1, Math.min(maxGridWidth, Math.round(correctedW)));
    correctedH = Math.max(1, Math.min(maxGridHeight, Math.round(correctedH)));
...
function calculateDisplaySize(
    ...
    // 매트 크기 제한 (셀 개수 × cm/셀)
    const maxWidthCm = maxGridWidth * CM_PER_CELL;
    const maxHeightCm = maxGridHeight * CM_PER_CELL;
    const scale = Math.min(1, maxWidthCm / W_cm, maxHeightCm / H_cm);
    W_cm *= scale;
    H_cm *= scale;
...
export function generateGridSequence(
    ...
    const pairs = generateGridCombinations(correctedW, correctedH, maxGridWidth, maxGridHeight);

    // 3. 각 조합에 displayWidth/displayHeight 추가
    const sequence: GridStep[] = [];

    pairs.forEach(pair => {
        const sizes = calculateDisplaySize(bodyHeight, shoulderWidth, pair.w, pair.h, maxGridWidth, maxGridHeight);
        sequence.push({
            width: pair.w,
            height: pair.h,
            ...sizes,
