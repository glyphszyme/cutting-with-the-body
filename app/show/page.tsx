...
export default function ShowPage() {
    ...
    useEffect(() => {
        const fetchInitial = async () => {
            ...
            const { data: configData } = await supabase
                .from('config')
                .select('px_per_cm_w, px_per_cm_h, font_size_ratio')
                .eq('id', 1)
                .single();
            if (configData) {
    ...
    useEffect(() => {
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    const newData = payload.new as ConfigData;
                    setConfig({
                        px_per_cm_w: newData.px_per_cm_w,
                        px_per_cm_h: newData.px_per_cm_h,
                        font_size_ratio: newData.font_size_ratio,
                    });
                }
            )
            .subscribe();
    ...
    }, [width, height, bodyParts]);

    // 실제 물리적 크기 (픽셀로 변환)
    const W_show_px = displayWidth * config.px_per_cm_w;
    const H_show_px = displayHeight * config.px_per_cm_h;

    // 글자 크기 계산 (한 셀당 크기)
    const charWidthPx = width > 0 ? W_show_px / width : 0;
    const charHeightPx = height > 0 ? H_show_px / height : 0;
    const fontSize = Math.min(charWidthPx, charHeightPx) * config.font_size_ratio;
